...
Used by the GitHub Actions workflow after Selenium E2E execution.
"""
...
def generate_summary(results, base_url, run_number, output_path):
    ...
    lines = [
        f"# {status_emoji} Selenium E2E Test Summary",
        f"",
        f"> **Build**: #{run_number}  ",
    ...
    print(f"   Total: {total} | Passed: {passed} | Failed: {failed} | Rate: {pass_pct}")


def main():
    parser = argparse.ArgumentParser(description="Generate test summary markdown")
    parser.add_argument("--results", required=True, help="Path to pytest JSON report")
    parser.add_argument("--output", required=True, help="Output summary.md path")
    ...
    if results is None:
        print("⚠️  No results file found, generating empty summary")
        results = {"summary": {"total": 0, "passed": 0, "failed": 0, "skipped": 0, "duration": 0}, "tests": []}

    run_num = args.run_number or args.build_number
    generate_summary(results, args.base_url or args.report_url, run_num, args.output)
